...
export default function VideoStudio(){
  ...
  async function checkProviders(){
    try{
      const r=await fetch('/api/provider-status',{cache:'no-store'});
      if(!r.ok) throw new Error('Provider status gagal dibaca.');
      setProviderStatus(await r.json());
  ...
  function go(next:Section){
    setSection(next);setDrawer(false);setStatus(`${next} workspace dibuka`);
    requestAnimationFrame(()=>document.getElementById('studio-workspace')?.scrollIntoView({behavior:'smooth',block:'start'}));
  }
  ...
  async function generate(){
    setLoading(true);setVideoUrl('');setStatus('Mengirim job ke Fal AI Wan 2.7…');
    try{
      if(engine==='FAL_AI' && providerStatus && !providerStatus.providers.FAL_AI.configured){
        throw new Error('FAL_KEY belum dikonfigurasi di Vercel. Tambahkan secret FAL_KEY lalu redeploy.');
      }
      const r=await fetch('/api/video',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({title:'AI Video Project',prompt,type:'Educational',language:'id',duration:clipDuration,aspectRatio:ratio,style,engine,resolution,characterDescription:character,subtitle:true})});
      const j=await r.json();if(!r.ok)throw new Error(j.error||'Render PRO gagal.');
      const url=j.job?.videoUrl||'';
      setScenes(j.scenes||[]);setVideoUrl(url);setStatus(url?'Video PRO berhasil dibuat dan siap dipreview.':(j.message||'Job selesai tanpa URL video.'));
      if(url) requestAnimationFrame(()=>document.getElementById('video-result')?.scrollIntoView({behavior:'smooth',block:'center'}));
    }catch(e){setStatus(e instanceof Error?e.message:'Render PRO gagal.');}
    finally{setLoading(false);}
  }

  function enhance(){
    setPrompt(p=>`${p}. Cinematic composition, realistic physical motion, consistent identity, detailed classroom environment, natural camera movement, realistic lighting, anatomically correct hands and face, clean composition.`);
    go('Prompt Enhancer');
  }
