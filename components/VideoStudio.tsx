...
export default function VideoStudio() {
  ...
  async function generate(){
    setLoading(true); setVideoUrl(''); setStatus('Menghubungkan ke engine PRO…');
    try{
      ...
      setScenes(j.scenes||[]); setVideoUrl(j.job?.videoUrl||''); setStatus(j.message||'Job PRO dibuat');
      if(j.job?.status==='PROCESSING'&&j.job?.id) void poll(j.job.id,(j.provider||engine) as VideoEngine);
    }catch(e){setStatus(e instanceof Error?e.message:'Render PRO gagal.');}
    finally{setLoading(false);}
  }
  ...
  async function poll(id:string,eng:VideoEngine){
    for(let i=0;i<60;i++){
      ...
      try{
        ...
        if(j.videoUrl) setVideoUrl(j.videoUrl); if(j.status==='COMPLETED'){setStatus('Video PRO selesai dibuat');return;}
        setStatus(`Rendering PRO… ${j.status||'PROCESSING'}`);
      }catch{setStatus('Menunggu status engine PRO…');}
    }
  }

  function enhance(){ setPrompt(p=>`${p}. Cinematic, natural motion, consistent identity, detailed environment, professional camera movement, realistic lighting.`); setSection('Prompt Enhancer'); setStatus('Prompt PRO ditingkatkan'); }
    ...
    <header className="sticky top-0 z-50 border-b border-white/10 bg-[#080812]/90 backdrop-blur-xl">
      <div className="mx-auto flex max-w-[1500px] items-center justify-between gap-3 px-3 py-3 sm:px-5 lg:px-6">
        <div className="flex min-w-0 items-center gap-3">
          ...
          <div className="min-w-0"><div className="truncate font-black">AI WORLD <span className="text-violet-300">STUDIO X</span></div><div className="hidden truncate text-xs text-white/40 sm:block">AI Video Generator PRO · By Ahmad Yurid Ardiansyah, S.Pd.</div></div>
        </div>
        <div className="hidden items-center gap-2 sm:flex"><span className="rounded-full border border-violet-400/30 bg-violet-500/10 px-3 py-1 text-xs text-violet-200">PRO WORKSPACE</span><span className="rounded-full border border-white/10 px-3 py-1 text-xs text-white/50">REAL AI</span></div>
    ...
    <main className="mx-auto grid max-w-[1500px] gap-4 p-3 pb-24 sm:p-5 lg:grid-cols-[260px_minmax(0,1fr)_310px] lg:p-6 lg:pb-8">
      ...
      <section className="min-w-0 space-y-4">
        <div className="glass rounded-3xl p-4 sm:p-6">
          <div className="flex flex-col gap-4 xl:flex-row xl:items-end xl:justify-between"><div className="min-w-0"><div className="text-[11px] font-bold uppercase tracking-[.25em] text-violet-300">{section}</div><h1 className="mt-2 text-3xl font-black leading-tight sm:text-4xl lg:text-5xl"><span className="gradient-text">Buat Video Profesional</span> dengan AI</h1><p className="mt-3 text-sm leading-6 text-white/55">Workflow produksi PRO: prompt, storyboard, scene, voice-over, subtitle, render, dan MP4.</p></div><div className="grid grid-cols-2 gap-2 sm:flex"><button onClick={storyboard} className="rounded-2xl bg-white px-4 py-3 text-sm font-bold text-black"><FileText size={16} className="mr-2 inline"/>Storyboard</button><button onClick={generate} disabled={loading} className="rounded-2xl bg-gradient-to-r from-violet-500 to-cyan-400 px-4 py-3 text-sm font-black disabled:opacity-50"><Sparkles size={16} className="mr-2 inline"/>Generate Video PRO</button></div></div>
          <div className="mt-5 grid gap-3 md:grid-cols-3"><Card label="STYLE"><select value={style} onChange={e=>setStyle(e.target.value)} className="mt-2 w-full bg-transparent text-sm outline-none">{styles.map(x=><option key={x}>{x}</option>)}</select></Card><Card label="ENGINE PRO"><select value={engine} onChange={e=>setEngine(e.target.value as VideoEngine)} className="mt-2 w-full bg-transparent text-sm outline-none"><option value="AUTO">AUTO · REAL AI</option><option value="VEO">VEO · PRO</option><option value="HUGGING_FACE">HUGGING FACE · PRO</option></select></Card><Card label="STATUS"><div className="mt-2 flex items-center gap-2 text-sm text-cyan-200"><i className={`h-2 w-2 rounded-full ${loading?'animate-pulse bg-amber-300':'bg-cyan-300'}`}/>{status}</div></Card></div>
        </div>

        {section !== 'AI Video Editor' && section !== 'AI Voice / TTS' && <div className="glass rounded-3xl p-4 sm:p-5"><div className="flex items-center justify-between"><b>1. Ide / Prompt</b><span className="text-xs text-white/30">PRO</span></div><textarea value={prompt} onChange={e=>setPrompt(e.target.value)} rows={5} className="mt-3 min-h-[125px] w-full rounded-2xl border border-white/10 bg-black/20 p-4 text-sm leading-6 outline-none focus:border-violet-400/40"/><div className="mt-2 flex flex-wrap gap-2"><button onClick={enhance} className="rounded-xl bg-violet-500/10 px-3 py-2 text-xs text-violet-200"><Wand2 size={14} className="mr-1 inline"/>Enhance Prompt</button><button onClick={()=>setPrompt('Video cinematic tentang guru mengajar di kelas, profesional, natural, inspiratif.')} className="rounded-xl bg-white/5 px-3 py-2 text-xs text-white/60">Random Prompt</button><button onClick={()=>setPrompt('')} className="rounded-xl bg-white/5 px-3 py-2 text-xs text-white/60">Clear</button></div></div>}
        ...
        <div className="glass rounded-3xl p-4 sm:p-5"><div className="mb-4 flex items-center justify-between"><b>3. Storyboard & Scene Queue</b><span className="text-xs text-white/35">{scenes.length?`${scenes.length} scene · ${total}s`:'Belum dibuat'}</span></div>{scenes.length===0?<div className="rounded-2xl border border-dashed border-white/10 p-8 text-center text-sm text-white/35">Generate storyboard PRO untuk membuat scene.</div>:<div className="space-y-3">{scenes.map((s,i)=><div key={s.id} className="rounded-2xl border border-white/10 bg-black/10 p-4"><div className="flex gap-3"><div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-violet-500/10 text-sm font-black text-violet-200">{i+1}</div><div className="min-w-0 flex-1"><div className="flex justify-between gap-3"><b className="break-words">{s.title}</b><span className="shrink-0 text-xs text-white/35">{s.duration}s</span></div><div className="mt-2 text-xs leading-5 text-white/45">{s.visualPrompt}</div></div><button onClick={()=>setStatus(`Scene ${i+1} siap di-regenerate PRO`)} className="h-9 w-9 shrink-0 rounded-xl bg-white/5"><RefreshCw size={14}/></button></div></div>)}</div>}</div>

        {(section==='AI Video Editor'||section==='AI Voice / TTS'||section==='Auto Subtitle'||section==='Background Music') && <div className="glass rounded-3xl p-5"><b>{section}</b><p className="mt-2 text-sm text-white/45">Workspace {section} aktif dalam mode PRO.</p><div className="mt-4 grid gap-3 sm:grid-cols-3"><button onClick={()=>setStatus(`${section} siap`)} className="rounded-xl bg-white/5 p-3 text-sm">Open Workspace</button><button onClick={()=>setStatus('Pengaturan tersimpan')} className="rounded-xl bg-white/5 p-3 text-sm">Save Settings</button><button onClick={()=>setStatus('Preview diperbarui')} className="rounded-xl bg-violet-500/15 p-3 text-sm">Preview</button></div></div>}
      </section>

      <aside className="min-w-0 space-y-4 xl:sticky xl:top-[82px] xl:self-start"><div className="glass rounded-3xl p-4"><div className="mb-3 flex justify-between"><b>Preview PRO</b><span className="text-xs text-white/30">{ratio}</span></div><div className="flex aspect-video items-center justify-center overflow-hidden rounded-2xl bg-gradient-to-br from-violet-950 via-slate-950 to-cyan-950">{videoUrl?<video src={videoUrl} controls playsInline className="h-full w-full object-contain"/>:<div className="text-center text-white/45"><Play size={26} className="mx-auto mb-2"/>Video Preview</div>}</div><div className="mt-3 grid grid-cols-2 gap-2"><div className="rounded-xl bg-white/5 p-2 text-center text-xs">{resolution}</div><div className="rounded-xl bg-white/5 p-2 text-center text-xs">{ratio}</div></div><a href={videoUrl||'#'} download className={`mt-3 flex items-center justify-center gap-2 rounded-xl p-3 text-sm ${videoUrl?'bg-cyan-500/15 text-cyan-200':'pointer-events-none bg-white/5 text-white/25'}`}><Download size={16}/>Download MP4</a></div><div className="glass rounded-3xl p-4"><b>Quick Actions PRO</b><div className="mt-3 grid grid-cols-2 gap-2"><button onClick={()=>setStatus('Upload asset PRO dibuka')} className="rounded-xl bg-white/5 p-3 text-xs"><Upload size={15} className="mx-auto mb-1"/>Upload</button><button onClick={()=>{setScenes([]);setVideoUrl('');setStatus('Project PRO baru')}} className="rounded-xl bg-white/5 p-3 text-xs"><Plus size={15} className="mx-auto mb-1"/>New</button></div></div></aside>
    </main>
