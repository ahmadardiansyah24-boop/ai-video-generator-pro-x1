...
export async function POST(req:Request){
  try{
    ...
    const provider=getProvider(data.engine||'AUTO');
    const job=await provider.generateTextToVideo(data,first);
    return NextResponse.json({message:`${provider.name}: ${job.status}`,provider:provider.key,scenes,job});
  }catch(e){
    return NextResponse.json({error:e instanceof Error?e.message:'Video generation gagal.'},{status:500});
  }
}
